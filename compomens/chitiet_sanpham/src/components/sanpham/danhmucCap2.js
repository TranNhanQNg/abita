...
//Animater cho danh mục
    ...
    const translateOpycity = scrollY.interpolate({
      ...
      const fechDaTa=()=>{
        ...
        var data_fech = JSON.stringify({KiemTra:MD5.home,Muc:'IdDanhMuc',IdDanhMuc:idDanhMuc})
        fechDaTa_Axios(api_fech,set_then,set_catch,set_finally,data_fech)
      }
      const fechDaTa1=()=>{
    ...
    const renderList = (type, item, index) => {
      return (
       ...
       load?
           <Animated.View style={{opacity:fadeAnimFlatlist,flex:1,justifyContent: 'center'
            }}>   
      ...
      const dieuhuong = (item)=>{ 
                        navigation.navigate("DanhMucCap3",
