...
export default function Chitiet_Mota ({navigation, chitiet,MaTinh,layindex,dataAnh}) {
...
const dieuhuong =() => {navigation.navigate("SanPhamThuongHieu",{
	...
	return(	
		<View>
			...
			<View style ={{flexDirection:'row',alignItems:'center', flexWrap:'wrap',marginLeft:10,marginVertical:15+layindex%2*2}}>
				<Text style={[text,{fontWeight:'bold', }]}>Danh mục: </Text>
				<TouchableOpacity onPress={()=>{ navigation.navigate("DanhMucCap2",
                        {idDanhMuc:chitiet.IDDANHMUC,tendanhmuc:chitiet.TENDANHMUC})}}>
					<Text style={{color:'blue'}}> ... </Text>
				</TouchableOpacity>
				<Text style={{color:'#000'}}> ➤ </Text>
				<TouchableOpacity onPress={()=>{ navigation.navigate("DanhMucCap3",
			...
			<View style={{flexDirection:'row',marginHorizontal:10,alignItems:'center',justifyContent:'space-between'}}>
				<Text style={[text,{fontWeight:'bold',marginVertical:15}]}>Cửa hàng:</Text>
				<TouchableOpacity onPress={()=>navigation.navigate("CuaHang",{MaUidNCC:chitiet.MAUIDNCC, MaTinh:MaTinh})}
			style={{ borderRadius:3,borderColor:'#1E90FF',borderWidth:1}}>
				<Text style={{margin:5, color:'#1E90FF'}}>XEM CỬA HÀNG</Text>
			</TouchableOpacity>
			</View>
			<View style={{marginHorizontal:10}}>
				<Ten_CuaHang item={chitiet} color={'#C6DEA9'} color1={'#F8F8F8'} MaTinh={MaTinh}
			...
			<View style={{flexDirection:'row',borderColor:'#F5F5F5',marginVertical:10}}>
				...
				<TouchableOpacity style={styles1.baohanh}
			...
			<View style={{ alignItems:'center',flexDirection:'row', height:40,backgroundColor:'#F8F8F8',}}>
				...
				<TouchableOpacity onPress={()=>dieuhuong()} style={{flex:1,alignItems:'center',flexDirection:'row'}}>
			...
			<View>
					...
					<Text 	numberOfLines ={NUM_OF_LINE}
		...
		<Modal_BaoHanh
        setModalVisibleBaoHanh={setModalVisibleBaoHanh}
        modalVisibleBaoHanh={modalVisibleBaoHanh}
        MaTinh={MaTinh}
      />	
		</View>
